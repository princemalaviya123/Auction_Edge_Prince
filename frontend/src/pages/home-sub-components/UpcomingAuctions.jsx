...
<section className="my-8">
  ...
  <div className="flex flex-wrap gap-6">
    <div className="bg-[#161613] w-full p-4 gap-10 rounded-md flex flex-col justify-between lg:flex-1 lg:h-auto lg:p-6 2xl:flex-none 2xl:basis-64 2xl:flex-grow 2xl:px-4 2xl:py-6">
      <span className="rounded-full bg-[#fdba88] text-white w-fit p-3">
        <RiAuctionFill />
      </span>
      <div >
        <h3 className="text-yellow-400 text-xl font-semibold mb-2 min-[480px]:text-xl md:text-2xl lg:text-3xl">
          Auctions For
        </h3>
        <div>
    ...
    <div className="flex flex-col gap-4 w-full lg:flex-1 2xl:flex-none 2xl:basis-64 2xl:flex-grow">
      {auctionsStartingToday.slice(0, 2).map((element) => (
        <Link
          to={`/auction/item/${element._id}`}
          key={element._id}
          className="w-full flex flex-col gap-4 bg-white p-4 rounded-md 2xl:gap-2 hover:shadow-lg transition-all duration-300"
        >
          <div className="flex items-center gap-2">
            <img
              src={element.image?.url}
            ...
            />
            <p className="font-light text-primary text-sm">{element.title}</p>
          </div>
          <div className="flex justify-between">
