...
export const Drawer = ({ setOpenDrawer, openDrawer }) => {
  ...
  return (
    <>
      {/* <section
        className={`fixed ${openDrawer && singlePaymentProof.userId ? "bottom-0" : "-bottom-full"
          }  left-0 w-full transition-all duration-300 h-full bg-[#00000087] flex items-end`}
      > */}
      <section
        className={`fixed ${openDrawer && singlePaymentProof.userId ? "bottom-0" : "-bottom-full"} 
  left-0 w-full h-full overflow-auto transition-all duration-300 bg-[#00000087] flex items-end`}
      >
