...
import { republishAuction, deleteAuction } from "../store/slices/auctionSlice";
...
const CardTwo = ({ imgSrc, title, startingBid, startTime, endTime, id }) => {
    ...
    return (
        <>
            <div className="basis-full bg-gray-900 text-white rounded-md group sm:basis-56 lg:basis-60 2xl:basis-80 shadow-lg hover:shadow-xl transition-all duration-300">
  <img
    src={imgSrc}
    alt={title}
    className="w-full h-48 sm:h-56 md:h-64 lg:h-72 xl:h-80 object-cover rounded-t-md transition-all duration-300 group-hover:scale-105"
  />
  <div className="px-4 pt-4 pb-4">
    ...
    <div className="flex flex-col gap-3 mt-5">
      <Link
        className="bg-[#d6482b] text-center text-white text-lg px-4 py-2 rounded-md transition-all duration-300 hover:bg-[#b03a22]"
        to={`/auction/details/${id}`}
      >
        View Auction
      </Link>
      <button
        className="bg-red-500 text-center text-white text-lg px-4 py-2 rounded-md transition-all duration-300 hover:bg-red-700"
      ...
      <button
        ...
        className={`text-center text-white text-lg px-4 py-2 rounded-md transition-all duration-300 ${
...
const Drawer = ({ setOpenDrawer, openDrawer, id }) => {
    ...
    return (
        ...
        <section
  className={`fixed ${openDrawer && id ? "bottom-0" : "-bottom-full"} left-0 w-full h-full bg-[#00000087] flex items-end transition-all duration-300`}
>
  <div className="w-full bg-[#1E1E1E] rounded-t-2xl shadow-lg transition-all duration-300">
    <div className="px-6 py-8 sm:max-w-[640px] sm:m-auto">
      <h3 className="text-[#D6482B] text-3xl font-semibold text-center mb-2">
        Republish Auction
      </h3>
      <p className="text-gray-400 text-center">
        Let's republish the auction with the same details but new starting and ending time.
      </p>

      <form className="flex flex-col gap-6 mt-6">
        {/* Start Time Input */}
        <div className="flex flex-col gap-3">
          <label className="text-lg text-gray-300">Republish Auction Start Time</label>
          <DatePicker
            selected={startTime}
            onChange={(date) => setStartTime(date)}
            showTimeSelect
            ...
            timeIntervals={15}
            dateFormat={"MMMM d, yyyy h:mm aa"}
            className="text-lg py-2 bg-transparent border-b border-gray-500 focus:outline-none w-full text-white"
          />
        </div>

        {/* End Time Input */}
        <div className="flex flex-col gap-3">
          <label className="text-lg text-gray-300">Republish Auction End Time</label>
          <DatePicker
            selected={endTime}
            onChange={(date) => setEndTime(date)}
            showTimeSelect
        ...
        <div className="flex flex-col gap-4">
          <button
            type="button"
            className="bg-[#3B82F6] w-full py-3 rounded-lg text-white font-semibold text-xl transition-all duration-300 hover:bg-[#2563EB]"
            onClick={handleRepbulishAuction}
          >
            {loading ? "Republishing..." : "Republish"}
          </button>
          ...
          <button
            type="button"
            className="bg-[#FBBF24] w-full py-3 rounded-lg text-white font-semibold text-xl transition-all duration-300 hover:bg-[#D6482B]"
            onClick={() => setOpenDrawer(false)}
          >
            Cancel
          </button>
