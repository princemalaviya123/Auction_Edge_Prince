...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  const requestBody = req.body; // Store req.body in a variable to avoid destructuring issues
  ...
  let startingBid = requestBody.startingBid;
  let startTime = requestBody.startTime;
  let endTime = requestBody.endTime;

   // Convert IST to Zulu time
   startTime = new Date(startTime + ' GMT+0530').toISOString();
   endTime = new Date(endTime + ' GMT+0530').toISOString();

   console.log("Start Time in Zulu:", startTime);
   console.log("End Time in Zulu:", endTime);
...
export const republishItem = catchAsyncErrors(async (req, res, next) => {
  ...
  // when one User Won Action but that don't want to pay for that item. 
...
//   const alreadyOneAuctionActive = await Auction.find({
//     createdBy: req.user._id,
//     endTime: { $gt: Date.now() }, // gt = Grater than
//   });
//   if (alreadyOneAuctionActive.length > 0) {
//     return next(new ErrorHandler("You already have one active auction.", 400));
//   }
...
//       image.tempFilePath,
//       {
//         folder: "MERN_AUCTION_ITEM_IMAGES",
//       }
//     );
//     if (!cloudinaryResponse || cloudinaryResponse.error) {
//       console.error(
...
//   // its validate id is like mongodb_id ?
//   if (!mongoose.Types.ObjectId.isValid(id)) {
//     return next(new ErrorHandler("Invalid Id format.", 400));
//   }
//   const auctionItem = await Auction.findById(id);
...
//     return next(new ErrorHandler("Auction not found.", 404));
//   }
//   // In Auction We Arrange Bidder in Sorting Form
//   const bidders = auctionItem.bids.sort((a, b) => b.amount - a.amount);
//   res.status(200).json({
//     success: true,
//     auctionItem,
...
//   // Pati Gayela ma Bids, and commissionCalculated Ma Hashe kaik Tene Empty karva
//   data.bids = [];
//   data.commissionCalculated = false;
//   data.currentBid = 0;
//   data.highestBidder = null;
